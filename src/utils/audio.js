...
import { getChordNotes } from './chordDetector';
...
class AudioManager {
  constructor() {
    ...
    // Audio instrument preset
    this.audioPreset = "piano"; // "piano", "rhodes", "synth", "musicbox"
    this.pianoSoundEnabled = true;
  }
  ...
  noteOn(midiNote) {
    if (!this.ctx || this.isMuted || !this.pianoSoundEnabled) return;
    ...
    if (this.audioPreset === "rhodes") {
      this.noteOnRhodes(midiNote, now);
    } else if (this.audioPreset === "synth") {
      this.noteOnRetroSynth(midiNote, now);
    } else if (this.audioPreset === "musicbox") {
      this.noteOnMusicBox(midiNote, now);
    } else {
      this.noteOnGrandPiano(midiNote, now);
    }
  }

  noteOnGrandPiano(midiNote, now) {
    ...
    // Route hammer / strings to master
  ...
  noteOnRhodes(midiNote, now) {
    const freq = this.midiNoteToFreq(midiNote);
    const voiceGain = this.ctx.createGain();
    voiceGain.gain.setValueAtTime(0.8, now);

    // Tine attack (Sine at high frequency, fast decay)
    const tine = this.ctx.createOscillator();
    tine.type = "sine";
    tine.frequency.setValueAtTime(freq * 4, now);
    const tineGain = this.ctx.createGain();
    tineGain.gain.setValueAtTime(0.25, now);
    tineGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.08);

    // Body (Triangle wave detuned for electric piano warmth)
    const osc1 = this.ctx.createOscillator();
    osc1.type = "triangle";
    osc1.frequency.setValueAtTime(freq, now);
    const osc1Gain = this.ctx.createGain();
    osc1Gain.gain.setValueAtTime(0, now);
    osc1Gain.gain.linearRampToValueAtTime(0.24, now + 0.005);
    osc1Gain.gain.setTargetAtTime(0, now + 0.005, 1.2);
    ...
    const osc2Gain = this.ctx.createGain();
    osc2Gain.gain.setValueAtTime(0, now);
    osc2Gain.gain.linearRampToValueAtTime(0.12, now + 0.005);
    osc2Gain.gain.setTargetAtTime(0, now + 0.005, 0.65);

    // Tremolo LFO auto-pan
    const tremolo = this.ctx.createOscillator();
    tremolo.frequency.setValueAtTime(6.2, now);
    const tremoloGain = this.ctx.createGain();
    tremoloGain.gain.setValueAtTime(0.25, now);

    tremolo.connect(tremoloGain);
    tremoloGain.connect(voiceGain.gain);

    tine.connect(tineGain); tineGain.connect(voiceGain);
    osc1.connect(osc1Gain); osc1Gain.connect(voiceGain);
    osc2.connect(osc2Gain); osc2Gain.connect(voiceGain);
  ...
  noteOnRetroSynth(midiNote, now) {
    ...
    const osc1Gain = this.ctx.createGain();
    osc1Gain.gain.setValueAtTime(0, now);
    osc1Gain.gain.linearRampToValueAtTime(0.12, now + 0.012);
    osc1Gain.gain.setTargetAtTime(0, now + 0.012, 1.5);
    ...
    const osc2Gain = this.ctx.createGain();
    osc2Gain.gain.setValueAtTime(0, now);
    osc2Gain.gain.linearRampToValueAtTime(0.12, now + 0.012);
    osc2Gain.gain.setTargetAtTime(0, now + 0.012, 1.5);

    // Dynamic sweeping filter
  ...
  noteOnMusicBox(midiNote, now) {
    const freq = this.midiNoteToFreq(midiNote);
    const voiceGain = this.ctx.createGain();
    voiceGain.gain.setValueAtTime(0.75, now);

    // Pure toy-piano chime sine oscillators
    const osc1 = this.ctx.createOscillator();
    osc1.type = "sine";
    osc1.frequency.setValueAtTime(freq, now);
    const osc1Gain = this.ctx.createGain();
    osc1Gain.gain.setValueAtTime(0, now);
    osc1Gain.gain.linearRampToValueAtTime(0.24, now + 0.002);
    osc1Gain.gain.setTargetAtTime(0, now + 0.002, 0.45);
    ...
    const osc2Gain = this.ctx.createGain();
    osc2Gain.gain.setValueAtTime(0, now);
    osc2Gain.gain.linearRampToValueAtTime(0.12, now + 0.002);
    osc2Gain.gain.setTargetAtTime(0, now + 0.002, 0.24);
    ...
    const osc3Gain = this.ctx.createGain();
    osc3Gain.gain.setValueAtTime(0, now);
    osc3Gain.gain.linearRampToValueAtTime(0.06, now + 0.002);
    osc3Gain.gain.setTargetAtTime(0, now + 0.002, 0.15);
  ...
  schedulePlay(beat, time) {
    ...
    if (beat === 0) {
      ...
      const chordInfo = getChordNotes(targetChord);
      if (chordInfo) {
